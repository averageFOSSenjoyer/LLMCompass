...
def test_topo(topo, lock):
    ...
    print(
        f"{topo}, {compute_area_mm2}, {io_area_mm2}, {compute_area_mm2 + io_area_mm2}"
    )
    ...
    auto_regression_latency_simulated = 0
    print(
        f"{topo}, {auto_regression_latency_simulated}"
    )
    init_latency_simulated = model_init.compile_and_simulate(system, "heuristic-GPU")
    print(
        f"{topo}, {init_latency_simulated}"
    )
    with lock:
        with open(f"ae/topo/topo_results_bs{batch_size}_init.csv", "a") as f:
            f.write(
                f"{topo}, {compute_area_mm2 + io_area_mm2}, {init_latency_simulated}, {model_init.simluate_log}\n"
            )
        with open(f"ae/topo/topo_results_bs{batch_size}_ar.csv", "a") as f:
            f.write(
                f"{topo}, {compute_area_mm2 + io_area_mm2}, {auto_regression_latency_simulated}, {model_auto_regression.simluate_log}\n"
            )


lock = Lock()
processes = [
    Process(target=test_topo, args=(topo, lock))
    for topo in ["FC", "RING"]
]
